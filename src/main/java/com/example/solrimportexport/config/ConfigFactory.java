package com.example.solrimportexport.config;
...
public class ConfigFactory {
    ...
    public static CommandLineConfig getConfigFromArgs(String[] args) throws ParseException {
        ...
        if (actionType == null) {
            throw new MissingArgumentException("actionType should be [" + String.join("|", CommandLineConfig.ActionType.getNames()) + "]");
        }
        ...
        for (CommandLineConfig.ActionType o : CommandLineConfig.ActionType.values()) {
        ...
        c.setDryRun(dryRun);
        if (skipCount != null) {
            c.setSkipCount(Long.parseLong(skipCount));
        }
    ...
    private static Set<SolrField> convertStringToSolrFieldSet(String includeFields) {
        return Pattern.compile(",")
                ...
                {
                    if (s.startsWith("*")) {
                        return new SolrField(s.substring(1), SolrField.MatchType.ENDS_WITH);
                    } else if (s.endsWith("*")) {
                        return new SolrField(s.substring(0, s.length() - 1), SolrField.MatchType.STARTS_WITH);
                    } else
                        return new SolrField(s, SolrField.MatchType.EQUAL);
                })
    ...
    private static CommandLine parseCommandLine(String[] args) throws ParseException {
        ...
        cliOptions.addOption(ACTION_TYPE[0], ACTION_TYPE[1], true, "action type [" + String.join("|", CommandLineConfig.ActionType.getNames()) + "]");
        ...
        if (cmd.hasOption("help") || cmd.hasOption("h")) {
            String header = "solr-indexer-json\n\n";
            String footer = "\nThankyou";
